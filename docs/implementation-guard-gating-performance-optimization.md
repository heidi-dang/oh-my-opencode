...
- Optimized filtering and iteration

**Key Optimizations:**
```typescript
// Before: String.includes() for each phrase
const hasSuspiciousClaim = SUSPICIOUS_PHRASES.some(sp => lowerText.includes(sp.phrase))

// After: Pre-compiled regex patterns
const SUSPICIOUS_PHRASES = new Map([
  [/pr created|pull request created|pr opened|pull request opened/gi, "git_safe"],
  // ... more patterns
])
```
