    ...
    return {
        "tool.execute.after": async (
            ...
            if (safetyCritical.includes(input.tool)) {
                ...
                const success = normalizeBoolean(meta.success)
                const verified = normalizeBoolean(meta.verified)
                const changedState = normalizeBoolean(meta.changedState)
                ...
                if (verified === false) {
                    throw new Error(`[Verification Unconfirmed] Tool ${input.tool} executed but verification failed or was inconclusive. Hallucination risk! Aborting for safety.`)
                }
