...
beforeEach(() => {
  ...
  spyOn(shared, "fetchAvailableModels" as any).mockResolvedValue(new Set(["anthropic/claude-sonnet-4-6"]))
  ...
  spyOn(modelResolver, "resolveModelWithFallback" as any).mockReturnValue({ model: "anthropic/claude-sonnet-4-6" })
})
...
describe("Prometheus category config resolution", () => {
  test("resolves ultrabrain category config", () => {
    ...
    expect(config?.model).toBe("openai/o3-mini")
...
describe("Plan agent model inheritance from prometheus", () => {
  test("plan agent inherits all model-related settings from resolved prometheus config", async () => {
    //#given - prometheus resolves to claude-sonnet-4-6 with model settings
    ...
    expect(agents.plan).toBeDefined()
    expect(agents.plan.mode).toBe("subagent")
    expect(agents.plan.model).toBe("anthropic/claude-sonnet-4-6")
