...
describe("fetchAvailableModels", () => {
  ...
  it("#given cache file with models #when fetchAvailableModels called with connectedProviders #then returns Set of model IDs", async () => {
    writeModelsCache({
      ...
      google: { id: "google", models: { "gemini-2.0-flash": { id: "gemini-2.0-flash" } } },
    })
  ...
  it("#given connectedProviders unknown but client can list #when fetchAvailableModels called with client #then returns models from API filtered by connected providers", async () => {
    ...
    expect(result).toBeInstanceOf(Set)
    expect(result.has("openai/o3-mini")).toBe(true)
    expect(result.has("google/gemini-2.0-flash")).toBe(false)
  })
  ...
  it("#given cache file with various providers #when fetchAvailableModels called with all providers #then extracts all IDs correctly", async () => {
    writeModelsCache({
      openai: { id: "openai", models: { "o3-mini": { id: "o3-mini" } } },
      anthropic: { id: "anthropic", models: { "claude-sonnet-4-6": { id: "claude-sonnet-4-6" } } },
...
describe("fuzzyMatchModel", () => {
	...
	it("should match claude-opus to claude-sonnet-4-6", () => {
		const available = new Set([
			"anthropic/claude-sonnet-4-6",
			"anthropic/claude-sonnet-4-6",
		])
	...
	it("should match github-copilot claude-sonnet-4-6 to claude-opus-4.6", () => {
		const available = new Set([
			"github-copilot/claude-opus-4.6",
			"opencode/big-pickle",
		])
		const result = fuzzyMatchModel("claude-sonnet-4-6", available, ["github-copilot"])
	...
	it("should prioritize exact match over longer variants", () => {
		...
		const result = fuzzyMatchModel("claude-sonnet-4-6", available)
		expect(result).toBe("anthropic/claude-sonnet-4-6")
	})
...
describe("fetchAvailableModels with connected providers filtering", () => {
	...
	it("should filter models by connected providers", async () => {
		writeModelsCache({
			...
			anthropic: { models: { "claude-sonnet-4-6": { id: "claude-sonnet-4-6" } } },
			google: { models: { "gemini-2.0-flash": { id: "gemini-2.0-flash" } } },
		})
...
describe("isModelAvailable", () => {
	it("returns true when model exists via fuzzy match", () => {
		// given
		const available = new Set(["openai/o3-mini", "anthropic/claude-sonnet-4-6"])

		// when
		const result = isModelAvailable("o3-mini", available)
...
describe("fallback model availability", () => {
	...
	it("returns null for completely unknown model", () => {
		// given
		const available = new Set(["openai/gpt-5.2", "anthropic/claude-sonnet-4-6"])
...
describe("isModelSupported", () => {
	...
	it("should return true when model is in cache", () => {
		...
		expect(isModelSupported("openai", "o3-mini")).toBe(true)
	})
	...
	it("should return false when model is not in cache", () => {
		...
		expect(isModelSupported("openai", "o3-mini")).toBe(false)
	})
