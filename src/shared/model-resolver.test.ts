  ...
  describe("priority chain", () => {
    test("returns userModel when all three are set", () => {
      ...
      const input: ModelResolutionInput = {
        ...
        systemDefault: "google/gemini-2.0-flash",
      }
...
describe("resolveModelWithFallback", () => {
  ...
  describe("Step 1: UI Selection (highest priority)", () => {
    test("returns uiSelectedModel with override source when provided", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["opencode/big-pickle", "anthropic/claude-sonnet-4-6", "github-copilot/claude-sonnet-4-6-preview"]),
      ...
      expect(logSpy).toHaveBeenCalledWith("[MODEL-RESOLUTION] Resolved via override", { requested: "opencode/big-pickle", resolved: "opencode/big-pickle" })
    })
    ...
    test("UI selection takes priority over config override", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        uiSelectedModel: "opencode/big-pickle",
        userModel: "anthropic/claude-sonnet-4-6",
        availableModels: new Set(["opencode/big-pickle", "anthropic/claude-sonnet-4-6"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
    ...
    test("whitespace-only uiSelectedModel is treated as not provided", () => {
      ...
      expect(logSpy).toHaveBeenCalledWith("[MODEL-RESOLUTION] Resolved via override", { requested: "anthropic/claude-sonnet-4-6", resolved: "anthropic/claude-sonnet-4-6" })
    })
  ...
  describe("Step 2: Config Override", () => {
    test("returns userModel with override source when userModel is provided", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["anthropic/claude-sonnet-4-6", "github-copilot/claude-sonnet-4-6-preview"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
    ...
    test("override takes priority even if model not in availableModels", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["custom/my-model", "anthropic/claude-sonnet-4-6"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
  ...
  describe("Step 3: Provider fallback chain", () => {
    test("tries providers in order within entry and returns first match", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["github-copilot/claude-sonnet-4-6-preview", "opencode/claude-opus-4-7"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
      ...
      // then
      expect(result!.model).toBe("github-copilot/claude-sonnet-4-6-preview")
      expect(result!.source).toBe("provider-fallback")
      expect(logSpy).toHaveBeenCalledWith("[MODEL-RESOLUTION] Resolved via provider-specific fallback chain", {
        provider: "github-copilot",
        model: "claude-sonnet-4-6",
        match: "github-copilot/claude-sonnet-4-6-preview",
      })
    })
    ...
    test("respects provider priority order within entry", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["anthropic/gpt-5.2", "openai/gpt-5.2", "anthropic/claude-sonnet-4-6", "google/gemini-2.0-flash"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
    ...
    test("uses fuzzy matching within provider", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        fallbackChain: [
          { providers: ["anthropic", "github-copilot"], model: "claude-sonnet" },
        ],
        availableModels: new Set(["anthropic/claude-sonnet-4-6", "github-copilot/claude-sonnet-4-6-preview"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
    ...
    test("skips fallback chain when not provided", () => {
      // given
      const input: ExtendedModelResolutionInput = {
        availableModels: new Set(["anthropic/claude-sonnet-4-6", "google/gemini-2.0-flash"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
    ...
    test("cross-provider fuzzy match is no longer supported (falls through)", () => {
      ...
      // then - should skip zai-coding-plan because cross-provider match is disallowed, and fall through to anthropic
      expect(result!.model).toBe("anthropic/claude-sonnet-4-6")
      expect(result!.source).toBe("provider-fallback")
    })

    test("prefers specified provider naturally (no cross-provider match needed)", () => {
      ...
      // then - should prefer zai-coding-plan (specified provider)
      expect(result!.model).toBe("zai-coding-plan/glm-5")
      expect(result!.source).toBe("provider-fallback")
    })

    test("match preserves variant from entry", () => {
      // given - entry has variant, model found via specfied provider
      const input: ExtendedModelResolutionInput = {
        fallbackChain: [
    ...
    test("tries next entry if no match found anywhere", () => {
  ...
  describe("Step 4: System default fallback (no availability match)", () => {
    test("returns system default when no availability match found in fallback chain", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["google/gemini-2.0-flash", "openai/gpt-5.2", "anthropic/claude-sonnet-4-6"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
      ...
      expect(result!.source).toBe("system-default")
      expect(logSpy).toHaveBeenCalledWith("[MODEL-RESOLUTION] Resolved via system-default", { requested: "google/gemini-2.0-flash", resolved: "google/gemini-2.0-flash" })
    })
    ...
    test("uses connected provider from fallback when availableModels empty but cache exists", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        fallbackChain: [
          { providers: ["anthropic", "openai"], model: "claude-sonnet-4-6" },
        ],
        availableModels: new Set(),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
      ...
      expect(result!.model).toBe("openai/claude-sonnet-4-6")
      expect(result!.source).toBe("provider-fallback")
      cacheSpy.mockRestore()
    })
    ...
    test("falls through to system default when no provider in fallback is connected", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(),
        systemDefaultModel: "anthropic/claude-sonnet-4-6-20251101",
      }
      ...
      expect(result!.model).toBe("anthropic/claude-sonnet-4-6-20251101")
      expect(result!.source).toBe("system-default")
      cacheSpy.mockRestore()
    })
    ...
    test("returns system default when fallbackChain is not provided", () => {
      // given
      const input: ExtendedModelResolutionInput = {
        availableModels: new Set(["google/gemini-2.0-flash", "openai/gpt-5.2"]),
        systemDefaultModel: "google/gemini-2.0-flash",
      }
  ...
  describe("Multi-entry fallbackChain", () => {
    test("resolves to claude-opus when OpenAI unavailable but Anthropic available (oracle scenario)", () => {
      // given
      const availableModels = new Set(["anthropic/claude-sonnet-4-6"])
    ...
    test("tries all providers in first entry before moving to second entry", () => {
      // given
      const availableModels = new Set(["google/gemini-2.0-flash"])
    ...
    test("falls through to system default when none match availability", () => {
      // given
      const availableModels = new Set(["system/default", "other/model"])
  ...
  describe("categoryDefaultModel (fuzzy matching for category defaults)", () => {
    test("applies fuzzy matching to categoryDefaultModel when userModel not provided", () => {
      // given - gemini-2.0-flash is the category default, but only gemini-2.0-flash-preview is available
      const input: ExtendedModelResolutionInput = {
        categoryDefaultModel: "google/gemini-2.0-flash",
        fallbackChain: [
          { providers: ["google", "github-copilot", "opencode"], model: "gemini-2.0-flash" },
        ],
        availableModels: new Set(["google/gemini-2.0-flash-preview", "anthropic/claude-sonnet-4-6"]),
        systemDefaultModel: "anthropic/claude-sonnet-4-6",
      }
      ...
      // then - should fuzzy match gemini-2.0-flash → gemini-2.0-flash-preview
      expect(result!.model).toBe("google/gemini-2.0-flash-preview")
    ...
    test("categoryDefaultModel uses exact match when available", () => {
      ...
      const input: ExtendedModelResolutionInput = {
        ...
        availableModels: new Set(["google/gemini-2.0-flash", "google/gemini-2.0-flash-preview"]),
        systemDefaultModel: "anthropic/claude-sonnet-4-6",
      }
    ...
    test("categoryDefaultModel works when availableModels is empty but connected provider exists", () => {
      ...
      // then - should use categoryDefaultModel verbatim since categoryDefault doesn't auto track preview transforms
    ...
    test("transforms gemini-3-flash in categoryDefaultModel for google connected provider", () => {
      ...
      // then - gemini-3-flash does not transform here
    ...
    test("does not double-transform categoryDefaultModel already containing -preview", () => {
      ...
      const cacheSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(["google"])
      const input: ExtendedModelResolutionInput = {
        categoryDefaultModel: "google/gemini-2.0-flash-preview",
        availableModels: new Set(),
        systemDefaultModel: "anthropic/claude-sonnet-4-5",
      }
