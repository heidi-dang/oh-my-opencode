...
const DEFAULT_AGENT_MODEL_REQUIREMENTS: Record<string, ModelRequirement> = {
  ...
  heidi: {
    fallbackChain: [
      ...
      { providers: ["google", "github-copilot", "opencode"], model: "gemini-1.5-flash" },
      { providers: ["opencode"], model: "minimax-m2.5-free" },
      { providers: ["openai-compatible"], model: "llama3.2:latest" },
      { providers: ["openai-compatible"], model: "qwen2.5:latest" },
      { providers: ["openai-compatible"], model: "deepseek-coder:latest" },
      { providers: ["openai-compatible"], model: "codellama:latest" },
    ],
    requiresAnyModel: true,
  },
  "sisyphus-junior": {
